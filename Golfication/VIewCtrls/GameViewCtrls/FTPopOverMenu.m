...
#define FTDefaultMenuArrowHeight    0.0
#define FTDefaultMenuArrowWidth     0.0
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier menuName:(NSString *)menuName iconImageName:(NSString *)iconImageName textColor:(UIColor *)textColor
{
    ...
    if (self) {
        ...
        if (iconImage) {
            ...
            [_iconImageView setUserInteractionEnabled:FALSE];
        ...
        [_menuNameLabel setUserInteractionEnabled:FALSE];
...
-(void)adjustPopOverMenu
{
    ...
    if (arrowDirection == FTPopOverMenuArrowDirectionUp) {
        menuRect = CGRectMake(menuX-50, (senderRect.origin.y + senderRect.size.height) + 5, self.preferedWidth+50, menuHeight);
    ...
    UIBezierPath *shadowPath = [UIBezierPath bezierPathWithRoundedRect:_popMenuView.bounds cornerRadius:3.0];
    _popMenuView.layer.masksToBounds = NO;
    _popMenuView.layer.shadowColor = UIColor.lightGrayColor.CGColor;
    _popMenuView.layer.shadowOffset = CGSizeMake( 1.0, 1.0);
    _popMenuView.layer.shadowOpacity = 0.15;
    _popMenuView.layer.shadowPath = shadowPath.CGPath;
