...
- (void)addCoordinate:(ARGeoCoordinate *)coordinate {
   ...
   /* if (@available(iOS 11.0, *)) {
        scnView = [[SCNView alloc] init];
    } else {
        // Fallback on earlier versions
    }
    SCNScene *scene = [[SCNScene alloc] init];
    
    scnView.frame = ar_overlayView.frame;
...
//    float x =  (ob.bounds.size.width/360.0) * (180.0 + 28.517357721172367);
//    float y =  (ob.bounds.size.height/360.0) * (90.0 - 77.19895362854004);
    //https://stackoverflow.com/questions/45261016/convert-latitude-and-longitude-to-scnvector3-in-swift
//    float x =  (200/360.0) * (180.0 + 28.517357721172367);
//    float y =  (100/360.0) * (90.0 - 77.19895362854004);
    //https://forums.developer.apple.com/thread/23360
    CGFloat sinLong = sin((77.19895362854004) * M_PI / 180.0);
    CGFloat cosLong = cos((77.19895362854004) * M_PI / 180.0);
    CGFloat sinLat = sin((28.517357721172367) * M_PI / 180.0);
    CGFloat cosLat = cos((28.517357721172367) * M_PI / 180.0);
    
    CGFloat sinLong1 = sin((77.19903845340014) * M_PI / 180.0);
    CGFloat cosLong1 = cos((77.19903845340014) * M_PI / 180.0);
    CGFloat sinLat1 = sin((28.5173220748182) * M_PI / 180.0);
    CGFloat cosLat1 = cos((28.5173220748182) * M_PI / 180.0);

    SCNVector3 positions[] = {
        SCNVector3Make(6371000*sinLong1*cosLat1, 6371000*sinLat1, 6371000*cosLong1*cosLat1),
        SCNVector3Make(6371000*sinLong*cosLat, 6371000*sinLat, 6371000*cosLong*cosLat)
    };
...
//    scnView.playing = true;
    [ar_overlayView addSubview:scnView];
//    [ar_overlayView bringSubviewToFront:scnView];*/
    // --------------------------------------------------------
    ...
    [path addQuadCurveToPoint:CGPointMake(100,0) controlPoint:CGPointMake(25,100)];
    [path addLineToPoint:CGPointMake(99,0)];
    [path addQuadCurveToPoint:CGPointMake(1,0) controlPoint:CGPointMake(25,98)];
    [path closePath];
    
    SCNShape *shape = [SCNShape shapeWithPath:path extrusionDepth:0.75];
    shape.firstMaterial.diffuse.contents = SKColor.cyanColor;
    SCNNode *curveNode = [SCNNode nodeWithGeometry:shape];
    
    curveNode.position = curveNode.position;
    
    curveNode.rotation = SCNVector4Make(<#float x#>, <#float y#>, <#float z#>, <#float w#>);
    [scnView.scene.rootNode addChildNode:curveNode];*/
}

-(void) renderer:(id<SCNSceneRenderer>)renderer willRenderScene:(nonnull SCNScene *)scene atTime:(NSTimeInterval)time{
    /*if (lineNode == nil) {
        lineNode = [[SCNNode alloc] init];
        [scnView.scene.rootNode addChildNode:lineNode];
        [ar_overlayView addSubview:scnView];
    }*/
}
...
- (void) frontPositioning:(ARObjectView *)theView atCoordinate:(ARGeoCoordinate *)coord {
	if (theView.displayed && [self viewportContainsCoordinate:coord]) {
		...
		if (!(theView.superview)) {
            ...
            // ----------------------- My Code ---------------------------
            NSString *text = coord.dataObject;
            double distance = coord.baseDistance;
            
            NSString *suffix = @"meter";
            if(settingUnit != 1){
            ...
            UILabel *lbl = nil;
            for (lbl in theView.subviews){
                if ([lbl isKindOfClass:[UILabel class]]){
                    lbl.text = [NSString stringWithFormat:@"%@\n%d %@", text, finalDistance, suffix];
                    //lbl.text = text
                }
            }
            // ----------------------------------------------------------
